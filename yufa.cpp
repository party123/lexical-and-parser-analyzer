...
int gy(int a){
	int sum=a;
	int b[4],c,d,e,t=0,z=0,x,y,count1=0,count2=0;
	for(int i=3;i>=0;i--){
		b[i]=a%10;
		a/=10;
	}
	for(int j=0;j<4;j++)
		for(int k=0;k<4;k++){
			...
			for(int p=0;p<4;p++)
				if(j!=k&&k!=p&&j!=p){
					c=b[j]*100+b[k]*10+b[p];
					//cout<<c<<endl;
					for(int q=0;q<4;q++)
					if(q!=j&&q!=k&&q!=p&&c*b[q]==sum) {
						count2++;
						cout<<c<<' '<<b[q]<<endl;
				}
				}
...
bool gg(int a){
	...
	for(int j=0;j<4;j++)
		for(int k=0;k<4;k++)
			if(j!=k&&b[j]==b[k]) return false;
	return true;
}
int main(){
	int n,m,a[30],count,sum;
	count=0;
	sum=0;
	for(int i=0;i<m;i++)
		a[i]=0;
	for(int j=m;j<m+n;j++)
		...
		for(int k=1;k<n+m;k++){		
		//cout<<a[k]<<' ';
	if(a[k-1]==0&&a[k]==1) count++;
		}
    next_permutation(a,a+m+n);
	int q=gy(count,sum);
}
